...
The `only_strata_with_events` endpoint parameter enables the user to trim the set of presented stratification levels to those that only contain events in either of the treatment arms. 
...
ep_spec_ex5_2 <- mk_endpoint_str(stratify_by = list(c("SEX")),
                                 ...)
```
...
```{r, eval = FALSE}
# Example of partial endpoint specification with a strata definition
ep_spec_ex5_2 <- mk_endpoint_str(stratify_by = list(c("SEX", "AGEGGR1")),
                                 ...)
```
...
```{r, eval = FALSE}
# Example of partial endpoint specification with a strata definition
ep_spec_ex5_3 <- mk_endpoint_str(stratify_by = list(),
                                 ...)
```
